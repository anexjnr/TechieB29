...
export default function About() {
  ...
  const valuesHeading = useMemo(() => {
    const v = typeof about?.valuesHeading === "string" ? about.valuesHeading.trim() : "";
    return v.length ? v : "Our Purpose & Values";
  }, [about?.valuesHeading]);

  const valuesSubheading = useMemo(() => {
    const v = typeof about?.valuesSubheading === "string" ? about.valuesSubheading.trim() : "";
    return v.length
      ? v
      : "The principles that guide everything we do and shape the way we work with our clients.";
  }, [about?.valuesSubheading]);

  const valuesCards = useMemo(() => {
    const cards = Array.isArray(about?.valuesCards) ? (about!.valuesCards as any[]) : [];
    return cards.length
      ? cards
      : [
          {
  ...
  return (
    <div className="min-h-screen">
      ...
      {/* Purpose & Values */}
      {((valuesHeading && valuesHeading.trim().length > 0) ||
        (valuesSubheading && valuesSubheading.trim().length > 0) ||
        (Array.isArray(valuesCards) && valuesCards.length > 0)) && (
        <Section
        ...
        >
          ...
          {Array.isArray(valuesCards) && valuesCards.length > 0 && (
            ...
            >
              {valuesCards.map((card: any, idx: number) => {
