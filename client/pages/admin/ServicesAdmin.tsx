...
export default function ServicesAdmin() {
  ...
  const createOrUpdate = async () => {
    try {
      const payload = { title, description: desc, enabled };
      if (editingId) {
        ...
        if (res.ok) { setEditingId(null); setTitle(''); setDesc(''); setEnabled(true); fetchItems(); }
      } else {
        ...
        if (res.ok) { setTitle(''); setDesc(''); setEnabled(true); fetchItems(); }
      }
    } catch (e) {
      console.error(e);
    }
      ...
      <div className="mt-4 space-y-4">
        <div className="rounded-md border border-primary/20 p-4 bg-black/5">
          ...
          <div className="mt-3">
            ...
            {editingId && <button onClick={() => { setEditingId(null); setTitle(''); setDesc(''); }} className="ml-3 text-sm text-primary/80">Cancel</button>}
          </div>
        </div>
        <div>
          {loading ? (
              ...
              {items.map((it) => (
                <li key={it.id} className="rounded-md border border-primary/20 p-3 flex items-start justify-between bg-black/5">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => { setEditingId(it.id); setTitle(it.title); setDesc(it.description); setEnabled(!!it.enabled); }} className="text-sm text-primary/80 hover:text-primary">Edit</button>
