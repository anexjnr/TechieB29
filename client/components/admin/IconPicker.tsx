  ...
  return (
    <div className="relative inline-block">
      <button
        type="button"
        onClick={() => setOpen((s) => !s)}
        className="inline-flex items-center gap-2 rounded-md border border-primary/30 bg-transparent px-3 py-2 text-sm text-primary"
      >
        {Selected ? (
          <Selected className="h-4 w-4" />
        ) : (
          <span className="text-sm text-primary/80">Icon</span>
        )}
      ...
      {open && (
        <div className="absolute z-40 mt-2 right-0 w-64 max-h-72 overflow-auto rounded-md border border-primary/20 bg-black/5 p-3">
          <input
            ...
            className="w-full mb-2 rounded-md bg-transparent border border-primary/30 px-2 py-1 text-primary"
          />
            ...
            {names.map((n) => {
              ...
              return (
                <button
                  ...
                  className="p-1 rounded-md hover:bg-primary/10 flex items-center justify-center"
                >
