...
router.get('/news', async (_req, res) => {
  try {
    const items = await prisma.news.findMany({ orderBy: { date: 'desc' }, include: { image: true } as any });
    res.json(items);
  } catch (e) {
...
router.get('/testimonials', async (_req, res) => {
  try {
    const items = await prisma.testimonial.findMany({ include: { avatar: true } as any });
    res.json(items);
  } catch (e) {
    console.warn('Prisma testimonials failed, using memory store', e.message || e);
...
router.get('/projects', async (_req, res) => {
  try {
    const items = await prisma.project.findMany({ include: { image: true } as any });
    res.json(items);
  } catch (e) {
...
router.get('/about', async (_req, res) => {
  try {
    const items = await prisma.about.findMany({ include: { image: true } as any });
    res.json(items);
  } catch (e) {
...
router.get('/sections', async (_req, res) => {
  try {
    const items = await prisma.section.findMany({ orderBy: { order: 'asc' }, include: { image: true } as any });
    res.json(items);
  } catch (e) {
