...
export default function ServicesAdmin() {
  ...
  const persistUpdate = async (id:string, payload:any) => {
    const res = await fetch(`/api/admin/services/${id}`, { method: 'PUT', headers: { 'Content-Type':'application/json', ...(token ? { Authorization: `Bearer ${token}` } : {}) }, body: JSON.stringify(payload) });
    if (res.ok) fetchItems();
  };
  ...
  const createOrUpdate = async () => {
    try {
      ...
      if (editingId) {
        ...
        setEditingId(null);
        setTitle(''); setDesc(''); setEnabled(true); setIcon(null); setFile(null); setImagePreview(null);
      } else {
        ...
        <div className="rounded-md border border-primary/20 p-4 bg-black/5">
          <div className="flex items-center justify-between">
            <div className="flex-1">
              ...
              <RichTextEditor value={desc} onChange={setDesc} placeholder="Description" />
            ...
            <div className="ml-4 flex flex-col gap-2">
              <button onClick={()=>{ setShowAddModal(true); setNewTitle(''); setNewDesc(''); setNewIcon(null); setNewFile(null); setNewPreview(null); setNewEnabled(true); }} className="rounded-md bg-primary px-3 py-2 text-sm font-semibold text-white">Add New Service</button>
              <button onClick={()=>{ if (originalItems) setItems(originalItems); }} className="rounded-md border border-primary/30 px-3 py-2 text-sm">Reset Services</button>
            </div>
          </div>
          ...
          <div className="mt-3">
            ...
            {editingId && <button onClick={() => { setEditingId(null); setTitle(""); setDesc(""); setIcon(null); setEnabled(true); setFile(null); setImagePreview(null); }} className="ml-3 text-sm text-primary/80">Cancel</button>}
          </div>
          <div className="mt-2">
        ...
        <div>
          {loading ? (
            ...
            <ul className="space-y-3">
              {items.sort((a,b)=> (a.order ?? 0) - (b.order ?? 0)).map((it, idx) => {
                const Icon = it.icon ? getIconByName(it.icon) : null;
                return (
                  <li key={it.id} draggable onDragStart={(e)=>{ e.dataTransfer?.setData('text/plain', String(idx)); setDraggingIndex(idx); }} onDragOver={(e)=>{ e.preventDefault(); setDragOverIndex(idx); }} onDrop={async (e)=>{ e.preventDefault(); const from = Number(e.dataTransfer?.getData('text/plain')); const to = idx; setDraggingIndex(null); setDragOverIndex(null); if (isNaN(from)) return; if (from===to) return; const newItems = [...items]; const [moved] = newItems.splice(from,1); newItems.splice(to,0,moved); setItems(newItems); try{ await Promise.all(newItems.map((item,i)=> fetch(`/api/admin/services/${item.id}`, { method: 'PUT', headers: { 'Content-Type':'application/json', ...(token?{ Authorization: `Bearer ${token}` }: {}) }, body: JSON.stringify({ order: i }) }))); fetchItems(); }catch(e){console.error(e);} }} className={`rounded-md border ${dragOverIndex===idx? 'border-primary/40':'border-primary/20'} p-3 flex items-start justify-between bg-black/5`}>
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-sm text-primary/80" dangerouslySetInnerHTML={{ __html: typeof it.description === 'string' ? it.description : '' }} />
                      </div>
                    </div>
          ...
          <div className="fixed inset-0 z-40 flex items-center justify-center">
            ...
            <div className="relative z-50 w-full max-w-2xl rounded-md bg-white/5 p-6">
              ...
              <div className="space-y-3">
                <input value={newTitle} onChange={(e)=>setNewTitle(e.target.value)} placeholder="Title" className="w-full rounded-md bg-transparent border border-primary/30 px-3 py-2 text-primary" />
                <RichTextEditor value={newDesc} onChange={setNewDesc} placeholder="Description" />
                <div className="mt-2 flex items-center gap-3">
                ...
                <input type="file" accept="image/*" onChange={(e)=>{ const f = e.target.files?.[0] ?? null; setNewFile(f); if (f) setNewPreview(URL.createObjectURL(f)); else setNewPreview(null); }} className="text-sm text-primary/80" />
                {newPreview && <img src={newPreview} alt="Preview" className="max-h-48 object-contain rounded-md border border-primary/20" />}
              </div>
              <div className="mt-4 flex justify-end gap-2">
                ...
                <button onClick={async ()=>{ try{ let uploaded:any=null; if (newFile){ uploaded = await uploadFile(newFile); } const payload:any = { title: newTitle, description: newDesc, enabled: newEnabled }; if (newIcon) payload.icon = newIcon; if (uploaded?.id) payload.imageId = uploaded.id; await persistCreate(payload); setShowAddModal(false); setNewTitle(''); setNewDesc(''); setNewIcon(null); setNewFile(null); setNewPreview(null); setNewEnabled(true); }catch(e){console.error(e);} }} className="rounded-md bg-primary px-3 py-2 text-sm font-semibold text-white">Add Service</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {confirmDeleteId && (
        <div className="fixed inset-0 z-40 flex items-center justify-center">
          ...
          <div className="relative z-50 w-full max-w-md rounded-md bg-white/5 p-6">
            ...
            <p className="mt-2 text-sm text-primary/80">Are you sure you want to delete this service? This action cannot be undone.</p>
            <div className="mt-4 flex justify-end gap-2">
