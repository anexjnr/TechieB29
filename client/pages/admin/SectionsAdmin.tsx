...
export default function SectionsAdmin() {
  ...
  const [navOriginalRows, setNavOriginalRows] = useState<
  ...
  useEffect(() => {
    ...
    if (nav) {
      ...
      try {
        const parsed =
          typeof nav.content === "string"
            ? JSON.parse(nav.content)
            : nav.content;
  ...
  const createOrUpdate = async () => {
    try {
      ...
      if (key === "flowchart" || key === "capabilities" || key === "what-we-do-compact") {
  ...
  const startEdit = (it: any) => {
    ...
    try {
      const parsed =
        typeof it.content === "string" ? JSON.parse(it.content) : it.content;
    ...
    if (it.imageId) {
      setImagePreview(`/api/admin/assets/${it.imageId}`);
    } else {
  ...
  const addCapability = () =>
    setCapabilities((prev) => [
      ...prev,
      { icon: "target", label: "New", desc: "" },
    ]);
  const updateCapability = (i: number, field: string, val: string) =>
    setCapabilities((prev) =>
    ...
    <div>
      ...
      <div className="mt-4 space-y-4">
        <div className="rounded-md border border-primary/20 p-4 bg-black/5">
          ...
          {key === "flowchart" || key === "capabilities" || key === "what-we-do-compact" ? (
            <div className="mb-2">
              <div className="flex items-center justify-between">
              ...
              <div className="mt-2 space-y-2">
                {capabilities.map((c, i) => (
                  <div
                    key={i}
                    className="rounded-md border border-primary/20 p-2 bg-black/5 flex items-start gap-2"
                  >
                    <IconPicker
                      value={c.icon || null}
                    ...
                    <input
                      ...
                      placeholder="label"
                      className="w-36 rounded-md bg-transparent border border-primary/30 px-2 py-1 text-primary"
                    />
                    <input
                      value={c.desc || ""}
                      onChange={(e) =>
                        updateCapability(i, "desc", e.target.value)
                      }
                      placeholder="desc"
                      className="flex-1 rounded-md bg-transparent border border-primary/30 px-2 py-1 text-primary"
                    />
                    <button
          ...
          <input
            ...
            className="w-full mt-2 mb-2 rounded-md bg-transparent border border-primary/30 px-3 py-2 text-primary"
          />
                ...
                <div className="space-y-3">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      const f = e.target.files?.[0] ?? null;
                      setNewFile(f);
                      if (f) setNewImagePreview(URL.createObjectURL(f));
                  ...
                  onClick={async () => {
                    try {
                      const res = await fetch(
                        `/api/admin/sections/${confirmDeleteId}`,
                        {
                          method: "DELETE",
                          headers: token
                            ? { Authorization: `Bearer ${token}` }
        ...
        <div>
          ...
          <div className="mt-2 rounded-md border border-primary/20 p-3 bg-black/5">
                  ...
                  <tr key={i} className="border-t border-primary/10">
                    <td className="py-2">
                      <input
                        ...
                        className="w-full rounded-md bg-transparent border border-primary/30 px-2 py-1 text-primary"
                      />
                    </td>
                    <td className="py-2">
                      <input
            ...
            <div className="mt-3 flex items-center gap-2">
              <button
                onClick={() =>
                  setNavRows((prev) => [...prev, { label: "New", link: "/" }])
                }
                className="rounded-md border border-primary/30 px-3 py-2 text-sm"
              >
                Add Row
              ...
              <button
                onClick={async () => {
                  try {
                    const payload = {
                      key: navSectionId ? undefined : "nav",
                      heading: "Navigation",
                      content: JSON.stringify(navRows),
                      enabled: true,
                    } as any;
                    if (navSectionId) {
                      await fetch(`/api/admin/sections/${navSectionId}`, {
                        method: "PUT",
                        headers: {
                          "Content-Type": "application/json",
                          ...(token
              ...
              <button
                onClick={() => setNavRows(navOriginalRows)}
                className="rounded-md border border-primary/30 px-3 py-2 text-sm"
              >
                Reset
              </button>
              ...
              <li
                ...
                onDrop={async (e) => {
                  ...
                  try {
                    await Promise.all(
                      newItems.map((item, i) =>
                        fetch(`/api/admin/sections/${item.id}`, {
                          method: "PUT",
                          headers: {
                            "Content-Type": "application/json",
                            ...(token
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-semibold text-primary">
                      {it.key} {it.heading ? `- ${it.heading}` : ""}
                    </div>
                    <div className="text-sm text-primary/80">
                      {typeof it.content === "string" && it.content.length > 100
