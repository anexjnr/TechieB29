  ...
  useEffect(() => {
    ...
    const fetchRecent = async () => {
      try {
        ...
        setRecent({
          news: Array.isArray(n) ? n.slice(0, 3) : [],
          projects: Array.isArray(p) ? p.slice(0, 3) : [],
        });
      } catch (e) {
        console.error(e);
      }
    ...
    <div>
      ...
      <div className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-4">
        {Object.entries(stats).map(([k, v]) => (
          <motion.div
            ...
            className="rounded-xl border border-primary/20 bg-black/10 p-4 text-center"
          >
            <div className="text-sm text-primary/80 capitalize">{k}</div>
            <div className="mt-2 font-extrabold text-primary text-2xl">
              {String(v)}
            </div>
          </motion.div>
        ))}
      ...
      <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-4">
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="rounded-xl border border-primary/20 bg-black/5 p-4"
        >
          ...
          <div className="mt-2 text-sm text-primary/80 space-y-2">
            {recent.news.length === 0 && recent.projects.length === 0 ? (
              ...
              <div>
                {recent.news.map((n: any) => (
                  <div key={n.id} className="text-sm text-primary/80">
                    News: {n.title}
                  </div>
                ))}
                {recent.projects.map((p: any) => (
                  <div key={p.id} className="text-sm text-primary/80">
                    Project: {p.title}
                  </div>
                ))}
              </div>
        ...
        >
          ...
          <div className="mt-2 flex flex-col gap-2">
            <a
              href="/admin/projects"
              className="px-3 py-2 rounded bg-transparent border border-primary/30 text-primary text-sm text-left"
            >
              Manage Projects
            </a>
            <a
