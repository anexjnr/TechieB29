...
export default function Careers() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16">
      <Section>
        <AnimatedTitle text="Careers" className="text-4xl sm:text-5xl font-extrabold text-foreground" />
        <p className="mt-4 max-w-prose text-foreground/85">Join a compact team that values clarity and craft.</p>
      </Section>
        ...
        {jobs.map((j, i) => (
          <div key={i} className="rounded-2xl border border-primary/20 bg-black/10 p-6 flex items-center justify-between">
            <div>
              ...
              <div className="text-sm text-foreground/75">{j.location} • {j.type}</div>
            </div>
            <a href="#apply" className="text-sm font-semibold text-foreground/80 hover:text-foreground">Apply →</a>
          </div>
        ))}
      </Section>
    </div>
