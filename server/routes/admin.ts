...
router.post("/users", requireRole(["ADMIN"]), async (req, res) => {
  ...
  try {
    ...
    const created = await prisma.adminUser.create({
      data: { email, passwordHash: pw, role: role || "EDITOR" },
    });
    res.status(201).json(created);
  } catch (e) {
    try {
      const created = createItem(
        memoryDb.admins as any,
        {
          email,
...
router.put("/users/:id", requireRole(["ADMIN"]), async (req, res) => {
  ...
  try {
    ...
    try {
      const updated = updateItem(memoryDb.admins as any, id, {
        ...(role ? { role } : {}),
...
router.post("/upload", upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    console.warn(
      "Prisma upload failed, falling back to disk storage",
      e.message || e,
    );
    try {
...
router.get("/policies", async (_req, res) => {
  try {
    const rows = await prisma.$queryRawUnsafe(
      'SELECT id, title, content, created_at as "createdAt" FROM policy_document ORDER BY created_at DESC',
    );
...
router.post("/policies", async (req, res) => {
  ...
  try {
    const inserted = await prisma.$executeRawUnsafe(
      "INSERT INTO policy_document (id, title, content, created_at) VALUES (gen_random_uuid()::text, $1, $2, now())",
      title,
      content || null,
    );
    // return fresh list
...
router.delete("/policies/:id", async (req, res) => {
  ...
  try {
    await prisma.$executeRawUnsafe(
      "DELETE FROM policy_document WHERE id = $1",
      id,
    );
    return res.json({ ok: true });
  } catch (e) {
...
router.get("/contact", async (_req, res) => {
  try {
    const rows = await prisma.$queryRawUnsafe(
      'SELECT id, name, email, message, created_at as "createdAt" FROM contact_inquiry ORDER BY created_at DESC',
    );
    return res.json(rows);
  } catch (e) {
...
router.delete("/contact/:id", async (req, res) => {
  ...
  try {
    await prisma.$executeRawUnsafe(
      "DELETE FROM contact_inquiry WHERE id = $1",
      id,
    );
    return res.json({ ok: true });
  } catch (e) {
...
router.get("/applications", async (_req, res) => {
  try {
    const rows = await prisma.$queryRawUnsafe(
      'SELECT id, name, email, position, resume_asset_id as "resumeAssetId", applied_at as "appliedAt" FROM application ORDER BY applied_at DESC',
    );
    return res.json(rows);
  } catch (e) {
  ...
  try {
    ...
    if (f) {
      const asset = await prisma.asset.create({
        data: { filename: f.originalname, mime: f.mimetype, data: f.buffer },
      });
      resumeAssetId = asset.id;
    }
    const inserted = await prisma.$executeRawUnsafe(
      "INSERT INTO application (id, name, email, position, resume_asset_id, applied_at) VALUES (gen_random_uuid()::text, $1, $2, $3, $4, now())",
      name,
      email,
      position || null,
      resumeAssetId,
    ...
    const created = createItem(
      memoryDb.applications as any,
      {
        name,
        email,
        position,
        resumeUrl: f ? `/uploads/${Date.now()}-${f.originalname}` : undefined,
...
router.get("/serve", async (_req, res) => {
  try {
    const rows = await prisma.$queryRawUnsafe(
      'SELECT id, title, description, "order" FROM serve_step ORDER BY "order" ASC',
    );
    return res.json(rows);
  } catch (e) {
...
router.post("/serve", async (req, res) => {
  ...
  try {
    await prisma.$executeRawUnsafe(
      'INSERT INTO serve_step (id, title, description, "order") VALUES (gen_random_uuid()::text, $1, $2, $3)',
      title,
      description || null,
      order || null,
    );
...
router.get("/scheduled_posts", async (_req, res) => {
  try {
    const rows = await prisma.$queryRawUnsafe(
      'SELECT id, news_id as "newsId", publish_at as "publishAt", status FROM scheduled_post ORDER BY publish_at DESC',
    );
    return res.json(rows);
  } catch (e) {
    return res.json([]);
...
router.post("/scheduled_posts", async (req, res) => {
  ...
  if (!newsId || !publishAt)
    return res.status(400).json({ error: "newsId and publishAt required" });
  try {
    await prisma.$executeRawUnsafe(
      "INSERT INTO scheduled_post (id, news_id, publish_at, status) VALUES (gen_random_uuid()::text, $1, $2, $3)",
      newsId,
      new Date(publishAt),
      "scheduled",
    );
...
router.get("/:section", async (req, res) => {
  ...
  try {
    ...
    console.warn(
      "Prisma unavailable, falling back to memory store for",
      section,
      e.message || e,
    );
    // fallback mapping
